...
export default function Places() {
  ...
  const uploadPhotoFiles = async (evt) => {
    ...
    try {
      ...
      setPhotos(prev => [...prev, ...res.data.filenames])
    } catch (error) {
      console.error(error)
    }
  }
      ...
      {action === 'new' && (
        <form onSubmit={formik.handleSubmit}>
          ...
          <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
            ...
            <label className="h-32 flex gap-1 items-center justify-center border bg-transparent rounded-2xl p-6 text-2xl text-gray-600">
